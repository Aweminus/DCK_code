...
from dck.host import resum_trigger_reached
...
class MultiTurnReactAgent(FnCallAgent):
        ...
        for attempt in range(max_tries):
            try:
                chat_response = client.chat.completions.create(
                    ...
                    seed=self.llm_generate_cfg.get('seed'),
                )
    ...
    def _run(self, data: str, model: str, summary_iteration: int, **kwargs) -> List[List[Message]]:
        ...
        while num_llm_calls_available > 0:
            ...
            # host trigger routed through dck.host so that every experimental
            # arm shares literally the same trigger code (DCK spec 5.8)
            should_summarize = ((RESUM and resum_trigger_reached(token_count, max_tokens, num_llm_calls_available)) or round % summary_iteration == 0) and num_llm_calls_available 
            if should_summarize: 
